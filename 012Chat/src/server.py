...
def groupChat(c_socket, addr):
    ...
    while True:
        try:
            ...
            decoded_data = data.decode('utf-8', errors='ignore')  #인코딩된 데이터를 디코딩하고, 에러 발생 시 무시

            # 금칙어 처리
            filtered_data = changeWord(decoded_data)
            print(sender_info, '-', decoded_data)  #오가는 메시지들 로깅
